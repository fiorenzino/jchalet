...
public class CalendarUtils extends GregorianCalendar {
	 ...
	 */
	public CalendarUtils(long millisecondTime) {
		super();
	 ...
	 */
	public CalendarUtils(int y, int m, int d) {
		super(y, m, d);
	}
	 ...
	 *      int, int)
	 */
	public CalendarUtils(int y, int m, int d, int h, int min, int s) {
		super(y, m, d, h, min, s);
	}
	 ...
	 *            the interval for calculation.
	 */
	public long diffDay(Calendar end) {
		long endL = end.getTimeInMillis()
				+ end.getTimeZone().getOffset(end.getTimeInMillis());
		long startL = this.getTimeInMillis()
				+ this.getTimeZone().getOffset(this.getTimeInMillis());
		return ((endL - startL) / MILLISECS_PER_DAY) + 1;
	}
