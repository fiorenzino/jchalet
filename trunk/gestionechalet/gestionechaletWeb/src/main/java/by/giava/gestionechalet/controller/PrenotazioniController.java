...
public class PrenotazioniController extends
	...
	public void calcolaPrezzoByServices() {
		...
		List<Preventivo> lista = tariffeRepository
				.getTariffeInPeriodForServiziPrenotati(getServizi(),
						getSearch().getObj().isSoloStagionali(), "" + getFila());
	...
	public void resetCalcolaPrezzo() {
		reset();
	}
	...
	public void calcolaPrezzo() {
		...
		if (getSearch().getObj().getFila() == null
			...
			try {
				setFila(Integer.valueOf(getSearch().getObj().getFila()));
			} catch (Exception e) {
				// TODO: handle exception
			}

		}
		if (getSearch().getObj().getDataAl()
				.before(getSearch().getObj().getDataDal())) {
			super.addFacesMessage("il periodo selezionato non e' valido!");
		}
		int num = 0;
		...
		if (num == 0) {
			// aggiungi errore
			super.addFacesMessage("non hai selezionato nessun servizio");
			return;
