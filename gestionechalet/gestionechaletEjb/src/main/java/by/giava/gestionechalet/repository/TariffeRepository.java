...
public class TariffeRepository extends BaseRepository<Tariffa> {
		...
		try {
			...
			if (soloStagionali) {
				...
				query = em
						.createQuery(
								ql
										+ " AND (t.dal <= :START AND t.dal <= :STOP AND t.al >= :START AND t.al >= :STOP ) ")
	...
	public List<Preventivo> getTariffeInPeriodForServiziPrenotati(
			List<ServizioPrenotato> servizi, boolean soloStagionali, String fila) {
		List<Preventivo> result = new ArrayList<Preventivo>();
		for (ServizioPrenotato servizioPrenotato : servizi) {
			if (servizioPrenotato.getServizio() != null
					&& servizioPrenotato.getServizio().getFila() != null
					&& !servizioPrenotato.getServizio().getFila().isEmpty()) {
				fila = servizioPrenotato.getServizio().getFila();
			}
			try {
				List<Tariffa> resultP = (List<Tariffa>) em
						.createQuery(
								"select distinct(t) from Tariffa t left join fetch t.costi ti where (t.stagionale = :STAGIONALE) AND"
										+
	...
	public Tariffa persist(Tariffa tariffa) {
		try {
			...
			case OMB:
				...
				if (tariffa.getFila() != null && !tariffa.getFila().isEmpty()) {
					ombrelloneSearch.setFila(tariffa.getFila());
				}
			...
			case SDR:
				...
				if (tariffa.getFila() != null && !tariffa.getFila().isEmpty()) {
					sdraioSearch.setFila(tariffa.getFila());
				}
			...
			case LET:
				...
				if (tariffa.getFila() != null && !tariffa.getFila().isEmpty()) {
					lettinoSearch.setFila(tariffa.getFila());
				}
			...
			case CAB:
				...
				if (tariffa.getFila() != null && !tariffa.getFila().isEmpty()) {
					cabinaSearch.setFila(tariffa.getFila());
				}
			...
			case SED:
				...
				if (tariffa.getFila() != null && !tariffa.getFila().isEmpty()) {
					sediaSearch.setFila(tariffa.getFila());
				}
